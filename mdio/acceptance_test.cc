...
TEST(Variable, name) {
  ...
  EXPECT_EQ(voided.value().get_variable_name(), "voided")
      << voided.value().get_variable_name();
}
...
TEST(Variable, namedDimensions) {
  ...
  EXPECT_EQ(voided.value().getMetadata()["dimension_names"].size(), 2)
...
TEST(Variable, dimensionUnits) {
  ...
  EXPECT_TRUE(f8.value().getMetadata().contains("dimension_units"))
      << f8.value().getMetadata();
  EXPECT_TRUE(voided.value().getMetadata().contains("dimension_units"))
      << voided.value().getMetadata();
}
...
TEST(Variable, chunkSize) {
  ...
  EXPECT_TRUE(voided.value().get_chunk_shape().status().ok())
      << voided.value().get_chunk_shape().status();
}
...
TEST(Variable, getCompressor) {
  ...
  EXPECT_TRUE(i2Json.value()["metadata"].contains("compressor"))
      << i2Json.value();
  EXPECT_TRUE(i4Json.value()["metadata"].contains("compressor"))
      << i4Json.value();
  EXPECT_TRUE(i8Json.value()["metadata"].contains("compressor"))
      << i8Json.value();
  EXPECT_TRUE(f2Json.value()["metadata"].contains("compressor"))
      << f2Json.value();
  EXPECT_TRUE(f4Json.value()["metadata"].contains("compressor"))
      << f4Json.value();
  EXPECT_TRUE(f8Json.value()["metadata"].contains("compressor"))
      << f8Json.value();
  EXPECT_TRUE(voidedJson.value()["metadata"].contains("compressor"))
      << voidedJson.value();
}
...
TEST(Variable, domain) {
  ...
  EXPECT_EQ(f8.value().dimensions().shape()[0], EXPECTED_SHAPE)
      << f8.value().dimensions();
  EXPECT_EQ(voided.value().dimensions().shape()[0], EXPECTED_SHAPE)
      << voided.value().dimensions();
...
TEST(Dataset, fillValue) {
  nlohmann::json j = nlohmann::json::parse(datasetManifest);
  auto ds = mdio::Dataset::from_json(j, "zarrs/acceptance",
  ...
  // , , , , , , , } auto dataRes =
  // tensorstore::StaticDataTypeCast<mdio::dtypes::byte_t>(vd.get_data_accessor());
...
TEST(Dataset, write) {
  ...
  auto castedImageHeaders = reinterpret_cast<mdio::dtypes::byte_t*>(
      imageHeadersData.value().get_data_accessor().data());
  ...
  // EXPECT_EQ(castedImageHeaders[0], std::byte(0xffffffffffff)) << "Struct
  // array was not correct value";
}
